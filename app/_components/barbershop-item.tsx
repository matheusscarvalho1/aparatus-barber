...
const BarbershopItem = ({ barbershop }: BarbershopItemProps) => {
 ...
 <Link href={`/barbershops/${barbershop.id}`}>
  <div className="relative rounded-xl min-h-50 min-w-72.5">
    ...
    <div className="absolute bottom-0 left-0 right-0 z-20 p-4">
      <h3 className="text-terciary text-lg font-bold">{barbershop.name}</h3>
      <p className="text-terciary-foreground text-xs">{barbershop.address}</p>
    </div>
  </div> 
 </Link>
 );
